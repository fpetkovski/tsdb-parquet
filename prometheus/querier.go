...
func (q querier) Select(_ bool, hints *storage.SelectHints, matchers ...*labels.Matcher) storage.SeriesSet {
	...
	chunksProjection := compute.NewConcurrent(
		compute.ProjectColumns(
			selection[0],
			q.sectionLoader,
			defaultChunksBatchSize,
			schema.ChunkBytesColumn,
		), 3)

	return newSeriesSet(labelColumns, labelsProjection, chunksProjection)
}
